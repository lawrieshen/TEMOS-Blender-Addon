    ...
    with torch.no_grad():
        ...
        # with Progress(transient=True) as progress:
            ...
            for index in range(cfg.number_of_samples):
                ...
                # Get vertices, rots, trans
                if cfg.jointstype == "vertices":
                    vertices, rots, trans = model(batch)
                    print(f"Data shape of rots: {rots.shape}") # Data shape of rots: torch.Size([25, 22, 3, 3]) # [T, J, 3, 3]
                    print(f"Data shape of trans: {trans.shape}") # Data shape of trans: torch.Size([25, 3]) # [T, 3]
                ...
                # Save motion npy data (baked with SMPL mesh) 
                ...
                # Save rots and trans for rigging
                rig_dir = path.parent / "rig_npz_output"
                rig_dir.mkdir(parents=True, exist_ok=True)

                # OR save both into one .npz file
                np.savez(rig_dir / f"{filename}_rig.npz", rots=rots_np, trans=trans_np)
