...
class TEMOS(BaseModel):
    ...
    def forward(self, batch: dict) -> List[Tensor]:
        ...
        print("==== SMPLDatastruct Info ====")
        print("rots:", datastruct_from_text.rots.rots.shape)
        print("trans:", datastruct_from_text.rots.trans.shape)
        print("joints:", datastruct_from_text.joints.shape)
        print("rfeats:", datastruct_from_text.rfeats.shape)
        # print("jfeats:", datastruct_from_text.jfeats.shape)
        
        joints = datastruct_from_text.joints # [B, T, J, 3]
        rots = datastruct_from_text.rots.rots # [B, T, J, 3, 3]
        trans = datastruct_from_text.rots.trans # [B, T, 3]


        padding_removed_joints = remove_padding(joints, batch["length"])[0] # [T, J, 3]
        padding_removed_rots = remove_padding(rots, batch["length"])[0] # [T, J, 3, 3]
        padding_removed_trans = remove_padding(trans, batch["length"])[0] # [T, 3]

        return padding_removed_joints, padding_removed_rots, padding_removed_trans
