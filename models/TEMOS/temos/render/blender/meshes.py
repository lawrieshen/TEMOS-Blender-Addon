...
class Meshes:
    ...
    def load_in_blender_with_return(self, index, mat, use_shape_keys=False):
        vertices = self.data[index]
        faces = self.faces
        name = f"{str(index).zfill(4)}"

        from .tools import load_numpy_vertices_into_blender_with_return
        return load_numpy_vertices_into_blender_with_return(vertices, faces, name, mat), name
...
def prepare_meshes(data, canonicalize=True, always_on_floor=False):
    ...
    if always_on_floor:
        print(f"always_on_floor: {always_on_floor}")
