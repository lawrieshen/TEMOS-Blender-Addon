...
def setup_renderer(denoising=True, oldrender=True):
    ...
    # -- Modified to male device dynmaic for MacOS -- #
    cycles_prefs = bpy.context.preferences.addons["cycles"].preferences

    available_device_types = {device.type for device in cycles_prefs.devices}

    if "METAL" in available_device_types:
        cycles_prefs.compute_device_type = "METAL"
    elif "CUDA" in available_device_types:
